...
int main(void) {
  ...
  numDays = (year * 365.25) + ((month - 1) * 365.25 / 12.0) + day;
  todayDays = (TODAY_Y * 365.25) + ((TODAY_M - 1) * 365.25 / 12.0) + TODAY_D;
  ageDays = todayDays - numDays;
  int ageDaysThousands = ageDays / 1000;
  int ageDaysHundreds = (int)ageDays % 1000;
  //https://www.geeksforgeeks.org/c-if-statement/
  if (ageDaysThousands == 0) {
    printf("You age in days: %d\n", ageDaysHundreds);
  } else {
    printf("You age in days: %d,%d\n", ageDaysThousands, ageDaysHundreds);
  }
}
